...
async def run_bot_with_retry():
    ...
    IMPORTANTE: o bot é criado UMA vez fora do loop de retry para evitar
    que as tasks (auto_update_loop, voice_sweep_loop) se acumulem em memória
    a cada reconexão, causando atualizações duplicadas e vazamento de RAM.
    """
    tentativa = 0
    ...
    while True:
        try:
            ...
            # Fecha a sessão WebSocket antes de reconectar.
            # NÃO recria o bot aqui: o objeto bot é reutilizado para evitar
            # acúmulo de tasks (auto_update_loop, voice_sweep_loop) em memória.
            try:
