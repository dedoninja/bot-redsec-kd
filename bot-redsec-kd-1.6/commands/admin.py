...
class RegisterModal(Modal):
    def __init__(self, bot: discord.Bot):
        ...
        self.add_item(InputText(
            label="Plataforma (EA, Epic, Steam, PSN ou Xbox)",
            placeholder="ea",
            required=True,
            max_length=5
        ))
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if platform_raw not in ['ea', 'psn', 'xbox', 'steam', 'epic']:
            await interaction.response.send_message(
                "❌ Plataforma inválida. Use **ea**, **psn**, **xbox**, **steam** ou **epic**.",
                ephemeral=True
            )
...
def setup_admin(bot: discord.Bot):
    ...
    async def force_update(ctx: discord.ApplicationContext):
        ...
        if logs_channel:
            await logs_channel.send(
                f"🔄 **Force Update** solicitado por {ctx.author.mention} (`{ctx.author.id}`)"
            )
    ...
    async def report_salas(ctx: discord.ApplicationContext):
        ...
        guild = bot.get_guild(SERVER_ID)
        if not guild:
            await ctx.followup.send("❌ Servidor não encontrado.")
            return
    ...
    async def force_sweep(ctx: discord.ApplicationContext):
        ...
        await ctx.respond("🧹 Varredura de salas iniciada...")
        guild = bot.get_guild(SERVER_ID)
        if not guild:
            await ctx.followup.send("❌ Servidor não encontrado.")
            return
    ...
    async def search_player(ctx: discord.ApplicationContext, discord_id: str = None, gamertag: str = None):
        await ctx.defer()
        if not discord_id and not gamertag:
            await ctx.followup.send("❌ Informe ao menos um: **discord_id** ou **gamertag**.")
            return
        ...
        if discord_id:
            if discord_id in users:
                ...
                await ctx.followup.send(f"❌ Discord ID `{discord_id}` não encontrado no registro.")
                return
        else:
            for uid, info in users.items():
                if info.get('gamertag', '').lower() == gamertag.lower():
                    ...
                    break
            if not found_id:
                await ctx.followup.send(f"❌ Gamertag `{gamertag}` não encontrada no registro.")
                return

        gt            = found_info.get('gamertag', '?')
        plat          = found_info.get('platform', 'ea')
        ...
        if data and data != "api_error":
            ...
            # Fallback: campos de conta mesmo sem stats
            ...
            if persona_id and nucleus_id:
                ...
                api_url = f"<https://api.gametools.network/bf6/stats/?name={gt}>"
    ...
    async def search_ids(ctx: discord.ApplicationContext, gamertag: str):
        ...
        try:
            ...
            for persona in personas:
                ...
                kd_emoji = "❌"
                try:
                    stats_url = f"https://api.gametools.network/bf6/stats/?playerid={persona_id}&nucleus_id={nucleus_id}"
